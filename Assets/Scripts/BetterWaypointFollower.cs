...
public class BetterWaypointFollower : MonoBehaviour {
    ...
    private void Start() {
        ...
        for (int i = 1; i < circuit.Waypoints.Length; i++)
            distances[i] = (circuit.Waypoints[i].position - circuit.Waypoints[i - 1].position).magnitude +
                           distances[i - 1];
        distances[circuit.Waypoints.Length] =
            (circuit.Waypoints[circuit.Waypoints.Length - 1].position - circuit.Waypoints[0].position).magnitude +
            distances[circuit.Waypoints.Length - 1];
...
public class BetterWaypointEditor : Editor {
    List<String> _monoBehaviours;
    int _totalMethods;

    public override void OnInspectorGUI() {
        BetterWaypointFollower script = (BetterWaypointFollower) target;
        script.circuitObject =
            EditorGUILayout.ObjectField("Waypoint Circuit", script.circuitObject, typeof(WaypointCircuit), true);
        if (script.circuitObject != null) {
            ...
            if (script.varyingSpeed) {
                ...
                for (int i = 0; i < script.waypointSpeedFactors.Length; i++)
                    script.waypointSpeedFactors[i] = EditorGUILayout.Slider("Waypoint " + i.ToString("D3") + " Factor",
                        script.waypointSpeedFactors[i], 0f, 2f);
                EditorGUILayout.Space();
                ...
                EditorGUILayout.HelpBox("Waypoint speed factors take effect as soon as you pass that point, "
                                        ...
                                        + "\nThe speed factor is exponential. For example, 30 to the power of 2 is 900.",
                    MessageType.Info);
            }
            ...
            if (script.invokeMethods) {
                ...
                for (int i = 0; i < script.invokeWaypointEnabled.Length; i++) {
                    script.invokeWaypointEnabled[i] =
                        EditorGUILayout.Toggle("Invoke At Waypoint " + i.ToString("D3") + "?",
                            script.invokeWaypointEnabled[i]);
                    if (script.invokeWaypointEnabled[i]) {
                        script.invokeObject[i] = EditorGUILayout.ObjectField("Game Object Target",
                            script.invokeObject[i], typeof(MonoBehaviour), true);
                        if (script.invokeObject[i] != null) {
                            Type type = script.invokeObject[i].GetType();
                            MethodInfo[] methods = type.GetMethods();
                            if (_monoBehaviours == null)
                                FillList();
                            string[] names = new string[methods.Length - _totalMethods + 1];
                            if (names.Length == 0) {
                                EditorGUILayout.HelpBox("You have no public methods in your game object."
                                                        + "\nAdd 'public' in front of the method you wish to invoke.",
                                    MessageType.Error);
                            }
                            else {
                                int l = 0;
                                for (int j = 0; j < methods.Length; j++)
                                    if (!_monoBehaviours.Contains(methods[j].Name)) {
    ...
    private void FillList() {
        _totalMethods = 0;
        _monoBehaviours = new List<string>();
        MethodInfo[] methods = typeof(MonoBehaviour).GetMethods();
        foreach (MethodInfo mi in methods) {
            _monoBehaviours.Add(mi.Name);
            _totalMethods++;
        }
    }
}
